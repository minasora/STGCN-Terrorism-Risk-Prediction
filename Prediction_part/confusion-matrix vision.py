...
class DrawConfusionMatrix:
    ...
    def drawMatrix(self, i):
        ...
        plt.colorbar()  # 色条
        plt.savefig('./' + province_name[i] + '_ConfusionMatrix.png',
                    bbox_inches='tight')  # bbox_inches='tight'可确保标签信息显示全
        plt.show()


cm = np.load('confusion_matrix.npy')
province_name = [str(i) for i in range(34)]

plot = DrawConfusionMatrix(labels_name=['Happen', 'Not Happen'], normalize=False)
